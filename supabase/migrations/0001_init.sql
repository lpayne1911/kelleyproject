...
-- ----------------------------------------------------------------------------
-- This migration is IDEMPOTENT: it is safe to run repeatedly and against a
-- project that already has some of these objects. It creates what is missing
-- and skips what already exists (enums, tables, indexes, triggers, policies,
-- the storage bucket, and the seeded packages).
--
-- Written to match the business plan deck and the auto-side-first 90-day launch
-- plan. The credit engine (Engine 01) is stubbed minimally and fleshed out in a
-- later migration once compliance work is done.
--
-- Security model: RLS is ON for every table with NO public policies. The public
-- "upload your quote" form writes through a server action using the service-role
-- key (which bypasses RLS). Anon/auth users get access later via explicit
-- policies when the client portal is built.
-- ============================================================================

-- ---------- Enums -----------------------------------------------------------
-- CREATE TYPE has no IF NOT EXISTS, so guard each one and ignore if present.
do $$ begin
...
create index if not exists quote_documents_deal_idx on quote_documents (deal_id);
...
create index if not exists credit_clients_lead_idx on credit_clients (lead_id);
drop trigger if exists credit_clients_set_updated_at on credit_clients;
...
drop policy if exists "packages are readable by anyone" on packages;
