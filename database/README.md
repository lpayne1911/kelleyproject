...
| `submissions` | crowdsourced intake ledger (moderated) | `submission_id`, `review_status`, `dedupe_key` |
...
## Crowdsourced submissions flow

```
intake CSV --ingest--> submissions(pending) --review--> approved/rejected
                                              --promote--> pricing_observations (canonical)
```

The engine only reads `pricing_observations`, so unmoderated submissions never affect a
price. `promote` resolves each submission's provider/vehicle against the existing tables
and writes a labeled observation (`source_url = "crowdsourced submission #N"`). See
